<?php
/**
 * Kognetiks AI Summaries - Keyguard Support - Ver 2.2.6
 *
...
// kognetiks_ai_summaries_upgrade_encrypted_api_keys('kognetiks_ai_summaries_fetch_stored_api_keys', 'kognetiks_ai_summaries_update_stored_api_key', $old_keyguard);

// XORs the given key with a salt key and returns the base64-encoded result - Ver 2.2.6
function kognetiks_ai_summaries_obfuscate_keyguard($key) {
...
function kognetiks_ai_summaries_deobfuscate_keyguard($obfuscated_key) {
...
// Retrieves the keyguard secret key and if it doesn't exist, a new key is generated and stored - Ver 2.2.6
function kognetiks_ai_summaries_generate_and_store_keyguard() {
    ...
    if (empty($obfuscated_key)) {
        ...
        // Obfuscate the key.
        $obfuscated_key = kognetiks_ai_summaries_obfuscate_keyguard($key);
    ...
    return kognetiks_ai_summaries_deobfuscate_keyguard($obfuscated_key);
...
//
function kognetiks_ai_summaries_encrypt_api_key_with_key($plaintext, $keyguard) {
...
function kognetiks_ai_summaries_decrypt_api_key_with_key($data, $keyguard) {
...
function kognetiks_ai_summaries_encrypt_api_key($plaintext) {

    $keyguard = kognetiks_ai_summaries_generate_and_store_keyguard();
    return kognetiks_ai_summaries_encrypt_api_key_with_key($plaintext, $keyguard);

}

// Convenience function to decrypt an API key using the stored secret key - Ver 2.2.6
function kognetiks_ai_summaries_decrypt_api_key($data, $option_name = null) {
    ...
    if (!is_array($decoded) || !isset($decoded['iv']) || !isset($decoded['encrypted'])) {
        ...
        switch ($kognetiks_ai_summaries_ai_platform_choice) {
            case 'OpenAI':
                $option_name = 'kognetiks_ai_summaries_openai_api_key';
                break;
            case 'NVIDIA':
                $option_name = 'kognetiks_ai_summaries_nvidia_api_key';
                break;
            case 'Anthropic':
                $option_name = 'kognetiks_ai_summaries_anthropic_api_key';
                break;
            case 'DeepSeek':
                $option_name = 'kognetiks_ai_summaries_deepseek_api_key';
                break;
            case 'Mistral':
            ...
            case 'Azure OpenAI':
                ...
                break;
            case 'Local':
                $option_name = 'kognetiks_ai_summaries_local_api_key';
                break;
            default:
                $option_name = null;
        }
...
// with the new secret key (retrieved via kognetiks_ai_summaries_generate_and_store_keyguard()). The function takes two callbacks:
// one to fetch all stored keys and one to update each key in storage.
//
function kognetiks_ai_summaries_upgrade_encrypted_api_keys(callable $fetch_keys_callback, callable $update_key_callback, $old_keyguard) {

    // Get the new secret key (this will generate one if it doesn't exist).
    $new_keyguard = kognetiks_ai_summaries_generate_and_store_keyguard();
    ...
    foreach ($stored_keys as $key_id => $encrypted_api_key) {
        ...
        $decrypted = kognetiks_ai_summaries_decrypt_api_key_with_key($encrypted_api_key, $old_keyguard);
        if ($decrypted === false) {
            // Log or handle the decryption error, then skip this key.
            continue;
        }
        // Re-encrypt using the new secret key.
        $new_encrypted = kognetiks_ai_summaries_encrypt_api_key_with_key($decrypted, $new_keyguard);
...
//     ];
//
function kognetiks_ai_summaries_fetch_stored_api_keys($chatbot_ai_platform_choice = null) {
    ...
    $stored_keys = array();
    $stored_keys = array(
        'kognetiks_ai_summaries_openai_api_key' => esc_attr( get_option('kognetiks_ai_summaries_openai_api_key') ),
        'kognetiks_ai_summaries_nvidia_api_key' => esc_attr( get_option('kognetiks_ai_summaries_nvidia_api_key') ),
...
function kognetiks_ai_summaries_update_stored_api_key($key_id, $new_encrypted_value) {
    ...
    if ($key_id == 'kognetiks_ai_summaries_openai_api_key') {
        update_option('kognetiks_ai_summaries_openai_api_key', $new_encrypted_value);
    } elseif ($key_id == 'kognetiks_ai_summaries_nvidia_api_key') {
        update_option('kognetiks_ai_summaries_nvidia_api_key', $new_encrypted_value);
    } elseif ($key_id == 'kognetiks_ai_summaries_anthropic_api_key') {
        update_option('kognetiks_ai_summaries_anthropic_api_key', $new_encrypted_value);
    } elseif ($key_id == 'kognetiks_ai_summaries_deepseek_api_key') {
...
//
function kognetiks_ai_summaries_upgrade_unencrypted_api_keys(callable $fetch_keys_callback, callable $update_key_callback) {
    ...
    foreach ($stored_keys as $key_id => $stored_value) {
        ...
        $new_encrypted_value = kognetiks_ai_summaries_encrypt_api_key_with_key($stored_value, $keyguard);
...
add_action('admin_init', 'kognetiks_ai_summaries_run_api_key_upgrade');

// Force the upgrade process to run - Ver 2.2.6
function kognetiks_ai_summaries_run_api_key_upgrade() {

    // Check if the upgrade has already run.
    if ( get_option('kognetiks_ai_summaries_upgraded') !== 'yes' ) {
        // For unencrypted keys, run the upgrade.
        kognetiks_ai_summaries_upgrade_unencrypted_api_keys('kognetiks_ai_summaries_fetch_stored_api_keys', 'kognetiks_ai_summaries_update_stored_api_key');
        
        // Optionally, if you need to upgrade keys encrypted with an old secret key,
        // call kognetiks_ai_summaries_upgrade_encrypted_api_keys here as well:
        // kognetiks_ai_summaries_upgrade_encrypted_api_keys('kognetiks_ai_summaries_fetch_stored_api_keys', 'kognetiks_ai_summaries_update_stored_api_key', $old_keyguard);
        
        // Mark that the upgrade has been performed.
        update_option('kognetiks_ai_summaries_upgraded', 'yes');
    }

}

// Sanitize the API key - Ver 2.2.6
function kognetiks_ai_summaries_sanitize_api_key($input) {
    ...
    if (!empty($input)) {
        $encrypted_key = kognetiks_ai_summaries_encrypt_api_key($input);
